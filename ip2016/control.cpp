...
void process_func (int value)
{
    ...
    switch (value)
    {
        ...
        case M_PROCESS_COMPOSITE: // enum #12
        {
            ...
            if (!quietMode)
                cerr << "Enter filename (bmp) of second image (string - no spaces) : ";
            cin  >> filename;
            Image* secondImage = new Image();
            secondImage->readBMP(filename);
            if (!quietMode)
                cerr << "Enter filename (bmp) of mask (string - no spaces) : ";
            cin  >> filename;
            Image* mask = new Image();
            mask->readBMP(filename);
            checkStream(cin);
        ...
        case M_PROCESS_CONVOLVE: // enum #33
        {
            if (!quietMode)
                cerr << "Enter filename of kernel (string - no spaces) : ";
            cin >> filename;
            ...
            for (int i=0; i<size; i++) {
                ...
                for (int j=0; j<size; j++) {
                    fscanf(fp, "%lf", &kernel[i][j]);
                }
            }
            resultImage = ip_convolve(currentImage, size, kernel);
            for (int i=0; i<size; i++) {
                delete[] kernel[i];
            }
            delete[] kernel;
        ...
        {
            int dx, dy;
            dx = getInt("Enter shift in x direction (int): ");
            dy = getInt("Enter shift in y direction (int): ");
            resultImage = ip_image_shift(currentImage, dx, dy);
            
            break;
        }
            
        case M_PROCESS_INTERPOLATE: // #32
        {
            int w1=currentImage->getWidth();
            int h1=currentImage->getHeight();
            double interpolationFactor;
            char filename[MAX_NAME];
            if (!quietMode)
                cerr << "Enter filename (bmp) of second image (string - no spaces), must be same size as current image: ";
            cin  >> filename;
            Image* secondImage = new Image();
            secondImage->readBMP(filename);
            ...
            if (w1==w2 && h1==h2) {
                if (!quietMode)
                    cerr << "Enter interpolation/extrapolation factor : ";
                cin >> interpolationFactor;
                resultImage = ip_interpolate(currentImage, secondImage, interpolationFactor);
                
            }
            else {
                
                cerr << "Sorry, the images are different sizes.\n";
            }
            delete secondImage;
            break;
        ...
        case M_PROCESS_SEPIA: // enum #17
            resultImage = ip_sepia(currentImage);
            break;
